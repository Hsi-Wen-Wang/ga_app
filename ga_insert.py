...
class GeneticAl(Problem):
    def __init__(self, pop_size=300, cross_rate=0.8, mutate_rate=0.1, iteration=20):
        super().__init__()
#------------------------------------------
        self.gene = []
        self.population_op = []
...
#------------------------------------------
        ...
        self.chrom_filename = 'chromosome.csv'
        self.process_time = 0
        self.report_content = []
#------------------------------------------
        self.insertFit = []
    ...
    def reportChrom(self):
        self.best_chrom_de = self.decoder(self.best_chrom_op, self.best_chrom_de)
        with open(self.chrom_filename, 'w+', newline = '') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(self.best_chrom_op)
            writer.writerow(self.best_chrom_de)
            writer.writerow(self.best_chrom_mc)
